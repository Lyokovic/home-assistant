...
class XmppNotificationService(BaseNotificationService):
    ...
    def __init__(self, sender, resource, password, recipients, tls, verify, room, hass):
        """Initialize the service."""

        jid = f"{sender}/{resource}"

        self.bot = SendNotificationBot(hass, jid, password, tls, room, verify)
    ...
    async def async_send_message(self, message: str = "", **kwargs: Any) -> None:
        ...
        recipients = kwargs.get(ATTR_TARGET, self.default_recipients)
        data = kwargs.get(ATTR_DATA)
        timeout = data.get(ATTR_TIMEOUT, XEP_0363_TIMEOUT) if data else None

        await self.bot.send_notification(recipients, text, data, timeout)


class SendNotificationBot(slixmpp.ClientXMPP):
    """Service for sending Jabber (XMPP) messages."""

    def __init__(self, hass, jid, password, use_tls, room,
                 verify_certificate):
        ...
        # Plugins for image/file sending
    ...
    async def start(self, event):
        ...
        if self.room:
            await self.plugin["xep_0045"].join_muc_wait(self.room, self.jid_,
                                                        seconds=0)

    async def send_notification(self, recipients, message, data, timeout):
        """Sends the notification."""
        # Sending image and message independently from each other
        if data:
            await self.send_file(recipients, data, timeout=timeout)
        if message:
            self.send_text_message(recipients, message)

    async def send_file(self, recipients, data, timeout=None):
        """Send file via XMPP.
        ...
        try:
            ...
            url = await self.upload_file(data, timeout=timeout)

            _LOGGER.debug("Upload success")
            for recipient in recipients:
                if self.room:
                    _LOGGER.debug("Sending file to %s", self.room)
    ...
    async def upload_file(self, data, timeout=None):
        ...
        if data.get(ATTR_URL_TEMPLATE):
            ...
            templ = template_helper.Template(data[ATTR_URL_TEMPLATE], self.hass)
            get_url = template_helper.render_complex(templ, None)
            url = await self.upload_file_from_url(get_url, data, timeout=timeout)
        elif data.get(ATTR_URL):
            url = await self.upload_file_from_url(data[ATTR_URL], data, timeout=timeout)
        elif data.get(ATTR_PATH_TEMPLATE):
            _LOGGER.debug("Got path template: %s", data[ATTR_PATH_TEMPLATE])
            templ = template_helper.Template(data[ATTR_PATH_TEMPLATE], self.hass)
            get_path = template_helper.render_complex(templ, None)
            url = await self.upload_file_from_path(get_path, data, timeout=timeout)
        elif data.get(ATTR_PATH):
            url = await self.upload_file_from_path(data[ATTR_PATH], data, timeout=timeout)
        else:
            url = None
    ...
    async def upload_file_from_url(self, url, data, timeout=None):
        ...
        result = await self.hass.async_add_executor_job(get_url, url)
    ...
    async def upload_file_from_path(self, path: str, timeout=None):
        ...
        input_file = await self.hass.async_add_executor_job(
                self._read_upload_file, path)
    ...
    def send_text_message(self, recipients, message):
        """Send a text only message to a room or a recipient."""
        try:
            if self.room:
                _LOGGER.debug("Sending message to room %s", self.room)
                self.send_message(mto=self.room, mbody=message,
                                  mtype="groupchat")
            else:
                for recipient in recipients:
